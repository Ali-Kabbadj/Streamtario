...
export function startServer(port: number) {
    ...
    if (process.env.APP_ENV === "development") {
        ...
        const certPath = path.resolve(
            __dirname,
            "../../../../local_dev_deps/certs/localhost+2.pem",
        );
        const httpsOptions = {
    ...
    app.post("/setup-stream", async (req, res) => {
        const { infoHash, announce, fileIndex, startTime, durationSeconds } =
            req.body;
        ...
        try {
            await torrServerClient.addTorrent(
                infoHash,
                announce,
                fileIndex,
                startTime ?? 0,
    ...
    app.get("/file-stats/:infoHash/:fileIndex", async (req, res) => {
        ...
        try {
            ...
            console.error(
                `[Controller] Error getting file stats for ${infoHash}/${fileIndex}:`,
                error,
            );
            sendError(res, 500, {
                type: "DAEMON_ERROR",
    ...
    app.use("/direct/:infoHash/:fileIndex", (req, res) => {
        ...
        proxyReq.on("error", (err) => {
            ...
            if (!res.headersSent) {
                res
                    .status(502)
                    .send("Bad Gateway: Could not connect to torrent daemon.");
            }
        });
        req.pipe(proxyReq);
    });
