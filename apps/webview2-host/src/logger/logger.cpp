...
void Logger::Cleanup()
{
    ...
    if (m_mpv_log_file.is_open()) { m_mpv_log_file.close(); }
                                  ...
                                  const std::string &message)
{
    ...
    if (level == LogLevel::MPV) { file_str = "mpv"; }
...
{
#ifdef _DEBUG
    if (full_log_message.empty()) return;
    ...
    if (size_needed > 0)
    {
        ...
        // Use the Wide version of OutputDebugString for correct emoji rendering
        OutputDebugStringW(wstr.c_str());
    }
#else
    if (is_mpv_log)
    {
        if (m_mpv_log_file.is_open()) { m_mpv_log_file << full_log_message; }
    }
    else
    {
        if (m_log_file.is_open()) { m_log_file << full_log_message; }
    }
#endif
}
                 ...
                 const std::string &message)
{
    ...
    {
        // Avoid logging errors before logger is even ready, unless it's the init message itself
        return;
    }
...
void Logger::LogMpv(const mpv_event_log_message *msg)
{
    ...
    // We can extract more context from mpv's prefix if needed
    std::string context = msg->prefix;
    // Remove trailing newline that mpv often adds
