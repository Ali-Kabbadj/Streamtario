...
async function startGateway() {
  ...
  if (process.env.APP_ENV === "development") {
    const keyPath = path.resolve(PROJECT_ROOT, '../../local_dev_deps/certs/localhost+2-key.pem');
    const certPath = path.resolve(PROJECT_ROOT, '../../local_dev_deps/certs/localhost+2.pem');
